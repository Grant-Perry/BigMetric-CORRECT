...
struct BigMetric_Watch_AppApp: App {
   ...
   /*  This is the root - @main view so @StateObject is the parent - all other views needing access to the ParentObject
    need to use @ObservedObject or @EnvironmentObject

    @StateObject is the parent initializer for the object to be utilized in all child views. In order to be part of the "family", any child view requiring access to any of the components in the parent's need to use an ObservedObject or EnvironmentObject to "attach" to the parent's (and subsequent child) view components.
